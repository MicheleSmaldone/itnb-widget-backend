...
def crawl_phoenix_site(start_urls, base_url, max_pages=50):
    """
    Crawl Phoenix Technologies website to discover all available pages
    """
    ...
    while to_visit and len(visited) < max_pages:
        ...
        try:
            ...
            for a in soup.find_all('a', href=True):
                ...
                # Only follow links within phoenix-technologies.ch domain
    ...
    output_file = os.path.join(output_dir, "phoenix_crawl_summary.json")
    
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    
    print(f"Phoenix crawl summary saved to {output_file}")
    print(f"Discovered {len(results)} pages")
    
    # Also save a simple URL list for easy reference
    url_list_file = os.path.join(output_dir, "phoenix_urls.txt")
...
def main():
    base_url = "https://phoenix-technologies.ch"
    start_urls = [
    ...
    crawl_phoenix_site(start_urls, base_url, max_pages=30)
